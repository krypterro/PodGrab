...
def main(argv):
    ...
    # Sample Data
    #arguments.sub_feed_url = 'https://coder.show/rss'
    ...
    if not has_error:
        if mode == MODE_UNSUBSCRIBE:
            ...
            for sub in subs:
                ...
                data = open_datasource(feed_url)
                if not data:
                    print("'" + str(feed_url) + "' for '" + str(feed_name.encode("utf-8")) + "' is not a valid feed URL!")
                else:
                    message = iterate_feed(data, mode, download_directory, todays_date, cursor, connection, feed_url)
                    print(message)
                    mail += message
            mail = mail + "\n\n" + str(total_items) + " podcasts totalling " + str(
                total_size) + " bytes have been downloaded."
...
def export_opml_file(cur, conn, cur_dir):
    ...
    for sub in subs:
        ...
        file_handle.writelines(
            "\t<outline title=\"" + feed_name + "\" text=\"" + feed_name + "\" type=\"rss\" xmlUrl=\"" + feed_url + "\" htmlUrl=\"" + feed_url + "\"/>\n")
...
def write_podcast(item, chan_loc, date, type):
    ...
    if os.path.exists(local_file):
        ...
        try:
            # Here's where the 403 comes in at
            # item_file = urllib.request.urlopen(item)

            download(local_file, item)
            #item_file = requests.get(item, headers = headers)
            #output = open(local_file, 'wb')
            # output.write(item_file.read())
            #output.write(item_file)
            #output.close()
...
def download(save_file, url):
    new_file_name = save_file
    pos1 = new_file_name.find('?')
    if pos1 > 0:
        new_file_name = new_file_name[:pos1]
    headers = {
